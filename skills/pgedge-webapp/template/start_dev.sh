...
# ANSI-C-quoted so the values are real ESC bytes that render in heredocs,
# echo, and printf without needing `-e`.
...
if [ ! -f "$SERVER_CONFIG" ]; then
    echo "${YELLOW}No config found, copying example to $SERVER_CONFIG${NC}"
...
if [ ! -f "$BIN_DIR/auth.db" ]; then
    echo "${YELLOW}Initializing auth database (seeding admin)...${NC}"
    "$SERVER_BIN" --init --config "$SERVER_CONFIG" >> "$SERVER_LOG" 2>&1
fi
...
for _ in $(seq 1 30); do
    ...
    if ! kill -0 "$SERVER_PID" 2>/dev/null; then
        echo "${RED}Server died during startup${NC}"
        tail -20 "$SERVER_LOG"
        exit 1
    fi
    sleep 1
